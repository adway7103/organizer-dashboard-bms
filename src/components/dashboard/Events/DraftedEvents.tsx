...
export function DraftedEvents() {
  ...
  const [eventsData, setEventsData] = React.useState<TableType[]>([]);

  const fetchData = async () => {
    try {
      const response = await fetchDraftedEvents();
  ...
  const handleTurnLive = async (eventId: string) => {
    try {
  ...
  const columns: ColumnDef<TableType>[] = [
    ...
    },
    {
      ...
      cell: ({ row }) => {
        ...
        return (
          <div
            className="flex gap-2 bg-[#ededed] py-1 max-xl:px-3 justify-center items-center rounded-3xl hover:cursor-pointer hover:shadow-lg hover:text-black"
    ...
    },
    {
      ...
      cell: ({ row }) => (
        <div
          className="flex gap-2 bg-[#ededed] py-1 max-xl:px-3 justify-center items-center rounded-3xl hover:cursor-pointer hover:shadow-lg hover:text-black"
          onClick={() => handleTurnLive(row.getValue("eventId") as string)}
        >
          <div>Turn Live</div>
          <svg
            xmlns="http://www.w3.org/2000/svg"
  ...
  return (
    <div className="sm:ml-8 mr-2 sm:mr-8">
      <h3 className="text-3xl font-semibold pl-4">Drafted Events</h3>
      <h4 className="text-xl font-medium pl-5 mt-4">Event List</h4>
